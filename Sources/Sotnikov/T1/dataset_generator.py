...
class Generator:
    ...
    def generate(self, dots_count, dist, density, radius, weights, absolute=False):
        """
        Creates instance of dataset generator.
        :param dots_count:
        :param dist:
        ...
        X = np.linspace(0 if absolute else -dist, dist, density)
        ...
        while len(dataset_X) < dots_count:
            x_rand = torch.empty(1).uniform_(0 if absolute else x_min, x_max).to(device)
